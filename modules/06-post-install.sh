...
echo ""
echo -e "${YELLOW}  IMPORTANTE: Ejecutar 'source ~/.bashrc' o abrir una nueva terminal${NC}"
echo -e "${YELLOW}  para que claude, node, opencode y los aliases estén en PATH.${NC}"
echo ""
...
echo "  ── 3. GITHUB CLI ───────────────────────────────────────────"
echo "     gh auth login"
echo "     # En WSL2/headless: seleccionar 'Login with a web browser',"
echo "     # copiar el código, y abrir la URL manualmente en el browser"
echo "     # del host. O seleccionar 'Paste an authentication token'"
echo "     # con un PAT de github.com/settings/tokens"
echo ""
echo "  ── 4. CLAUDE CODE ──────────────────────────────────────────"
...
echo ""
echo "     # Dagu (systemd system — UN solo service, ver configs/dagu.service):"
echo "     sudo systemctl start dagu"
